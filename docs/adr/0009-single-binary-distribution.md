...
gzipped and unpacked into the Store the first time the Toolchain is used. Installing is one command
...
- Binaries are large: ~90MB installed, ~50–70MB compressed, plus ~85MB once mise is unpacked. That is
  ...
  instructions.
- The Store gains its first tenant before the Store slice exists. `~/.harv/store/mise/<version>/mise`
  is deliberately the shape the Store will keep, so nothing has to be migrated later.
