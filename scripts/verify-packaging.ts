...
function checkVendoredMise(binary: string): Check {
  ...
  const unpacked = join(home, "store", "mise", claimed, "mise");
  ...
  return {
    ...
    expectations: [
      ...
      expect(
        "it is unpacked into the Store, not left somewhere ad hoc",
        existsSync(unpacked),
...
/** The newest published release, or null if there is not one yet. */
function publishedVersion(): string | null {
...
async function checkUpdateHint(published: string | null): Promise<Check> {
  if (published === null) {
    return {
      ...
      measurements: {},
      expectations: [expect("a newer release exists to be behind", null, "no published release yet")],
    };
  }
...
async function main(argv: string[]): Promise<number> {
  ...
  log(`${DIM}latest published release: ${published ?? "none"}${RESET}`);
  ...
  const runners: Array<[string, string, () => Check | Promise<Check>]> = [
    ...
    ["one-command-install", "One command installs harv from the published release", () => checkInstall(published)],
    ["update-hint", "`harv --version` hints when it is out of date", () => checkUpdateHint(published)],
  ];
  ...
  if (asJson) {
    console.log(JSON.stringify({ platform, published, ok: failures.length === 0, checks }, null, 2));
  } else {
    report(checks);
    console.log(
