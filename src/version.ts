 ...
 * offline is not an error worth a paragraph. It asks GitHub for the *latest*
 * release, which skips pre-releases — so an `-rc` never nags anyone, and the
 * only way to be told about one is to have gone looking.
 */
...
export function defaultUpdateCheckDeps(): UpdateCheckDeps {
  return {
    now: () => Date.now(),
    cachePath: join(harvHome(process.env), CACHE_FILE),
